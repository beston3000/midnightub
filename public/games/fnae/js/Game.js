...
class Game {
    ...
    initElements() {
        ...
        this.multiplayerBtn = document.getElementById('multiplayer-btn'); // FIXED: Button mapped here!
    ...
    updateVolumePercents() {
        const sliders = [
            ...
            this.jumpscareVolumeSlider, this.ventCrawlingVolumeSlider, this.masterVolumeSlider
        ];
    ...
    bindEvents() {
        ...
        // FIXED: Multiplayer Button Logic triggers the Lobby
        ...
        document.querySelectorAll('.ai-btn-minus').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.getElementById(`${aiName}-value`).textContent = slider.value;
            });
        });
        ...
        document.querySelectorAll('.ai-btn-plus').forEach(btn => {
            btn.addEventListener('click', () => {
                const aiName = btn.dataset.ai;
                const slider = document.getElementById(`${aiName}-slider`);
                slider.value = Math.min(20, parseInt(slider.value) + 1);
                document.getElementById(`${aiName}-value`).textContent = slider.value;
            });
        });
    }
    ...
    saveProgress() {
        if (this.state.currentNight > 1) localStorage.setItem('fnae_current_night', this.state.currentNight.toString());
    }
    ...
    async startCustomNight() {
        ...
        if (window.multiplayer && window.multiplayer.role === 'engineer') {
            window.multiplayer.sendEvent('START_GAME', { night: this.state.currentNight });
        }
    ...
    async startGame() {
        ...
        const autoEndTimeout = setTimeout(() => endCutscene(), 3000);
    }
    ...
    showTutorial(type = 'night1') {
        ...
        if (type === 'night2') {
            tutorialContent.innerHTML = `<h2>DEFEND YOURSELF AGAINST CRISPY</h2><p>CRISPY WILL TRY TO ATTACK YOU THROUGH THE VENTS IN CAM 1 AND CAM 2, SO IF YOU HEAR BANGING IN THE VENTS HEAD OVER TO THE CONTROL PANEL AND CLOSE THEM. AFTER CLOSING THEM YOU WILL HEAR BANGING AGAIN AFTER A FEW SECONDS WHICH MEANS HE LEFT THE VENTS. YOU MUST OPEN THE VENTS OTHERWISE YOU WILL DIE FROM LACK OF OXYGEN. CRISPY CAN BE LURED WITH THE AUDIOS BUT YOUR MAIN PRIORITY WITH THE AUDIO LURES SHOULD BE EPSTEIN.</p><button id="tutorial-got-it">GOT IT</button>`;
        } else if (type === 'night3') {
            tutorialContent.innerHTML = `<h2>DEFEND YOURSELF AGAINST STEPHEN HAWKING</h2><p>STEPHEN HAWKING ALWAYS STAYS AT CAM 6 AND HE IS NOT AFFECTED BY THE AUDIO LURES. ELECTROCUTE STEPHEN HAWKING EVERY ONCE IN A WHILE TO PREVENT HIM FROM LEAVING CAM 6.</p><button id="tutorial-got-it">GOT IT</button>`;
        } else {
            tutorialContent.innerHTML = `<h2>DEFEND YOURSELF AGAINST CRISPY'S BROTHER</h2><p>CRISPY'S BROTHER ALWAYS STARTS AT CAM 11. USE THE CAMERA'S AUDIO LURE TO KEEP CRISPY'S BROTHER FAR AWAY FROM YOU. MAKE SURE THE CAMERA YOU'RE PLAYING THE SOUND IN IS NEXT TO THE CAMERA WHERE CRISPY'S BROTHER IS. PLAYING SOUND IN ONLY ONE SPOT WILL NOT WORK IF YOU DO IT TWICE OR MORE IN A ROW. USING THE AUDIO LURE TOO MUCH WILL LEAD TO THE CAMERAS BREAKING. TO FIX THEM HEAD TO THE CONTROL PANEL AND RESTART THE CAMERAS LIKE YOU JUST DID. CRISPY'S BROTHER DOES NOT ATTACK THROUGH THE VENTS SO DON'T BOTHER CLOSING THEM FOR THIS NIGHT.</p><button id="tutorial-got-it">GOT IT</button>`;
        }
    ...
    showGoldenStephen() {
        ...
        setTimeout(() => goldenOverlay.remove(), 2000);
    }
    ...
    showNightIntro() {
        return new Promise((resolve) => {
            ...
            if (this.state.customNight && this.state.currentNight === 7) nightIntroText.textContent = 'CUSTOM NIGHT';
            else nightIntroText.textContent = `NIGHT ${this.state.currentNight}`;
            
            nightIntro.classList.remove('hidden');
            setTimeout(() => nightIntro.classList.add('fade-in'), 50);
    ...
    startGameLoop() {
        this.timeInterval = setInterval(() => {
            ...
            if (this.state.currentTime >= 6) {
                if (window.multiplayer && window.multiplayer.role === 'watcher') return; 
                this.winNight();
            }
        }, 60000);
    ...
    updatePower() {
        if (this.state.ventsClosed) {
            ...
            if (this.state.oxygen < 100) this.state.oxygen += 2;
        }
    ...
    toggleVents() {
        ...
        setTimeout(() => {
            ...
            if (window.multiplayer && window.multiplayer.role === 'engineer') {
                window.multiplayer.sendEvent('TOGGLE_VENTS', { closed: this.state.ventsClosed });
            }
    ...
    winNight() {
        if (window.multiplayer && window.multiplayer.role === 'engineer') {
            window.multiplayer.sendEvent('NIGHT_WIN');
        }
    ...
    showMainMenu() {
        ...
        // Show normal main menu, hide multiplayer lobby if it was open
        this.mainMenu.classList.remove('hidden');
        const lobby = document.getElementById('multiplayer-lobby');
        if(lobby) lobby.style.display = 'none';
        
        // Reset multiplayer status text
